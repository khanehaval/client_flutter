...
class AdvertisementService {
  ...
  Future<List<String>> getOptionsFromDatabase() async {
    ...
    if (jsonString != null) {
      ...
      // Extract 'label' from each 'Item' in the 'list' of each 'Data'
      return baseModel.data?.expand((data) {
            // Ensure 'list' is not null and map through each 'Item' to extract the 'label'
            return (data.list ?? [])
                .map((item) =>
                    item.label ?? '') // Use 'label' or empty string if null
                .toList();
          }).toList() ??
          [];
    }
