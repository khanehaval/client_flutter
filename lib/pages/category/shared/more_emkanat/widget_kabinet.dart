...
void Kabinet(Function(String key, String label) onSelected) async {
  final RxInt index = 2.obs; // Default index
  final advertisementService = AdvertisementService();
  final Base? baseData = await advertisementService.fetchDataFromServer();

  // استخراج داده‌ها
  final Data? cabinetsData = baseData?.data?.firstWhere(
    (data) => data.key == "cabinets",
    orElse: () => Data(list: []),
  );
  final List<Item>? items = cabinetsData?.list;
